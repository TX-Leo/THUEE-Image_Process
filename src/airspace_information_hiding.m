function airspace_information_hiding(image_src,JpegCoeff_src)
    ...
    % image = hall_gray;
    load(JpegCoeff_src);% DCTAB, ACTAB, QTAB
    %load('../resource/JpegCoeff.mat');% DCTAB, ACTAB, QTAB
    %image = imread('D:\图片\Saved Pictures\logo\TX-Leo1.png');
    %save('image.mat','image');
    %load('image.mat');
    % information要隐藏的信息(height*width的大小)
    [a, b] = size(image);
    information = uint8(randi([0, 1], a, b));% 01随机序列
    % information = uint8(ones([a, b])); % 全1序列
    % information = uint8(zeros([a, b])); %全0序列
    % 对信息进行空域隐藏
    my_airspace_information_hiding_technology(information,image,DCTAB, ACTAB, QTAB);
end
...
function [h_res, w_res, image_res] = my_image_padding(height, width, h_base, w_base, image)
    ...
    if mod(height, h_base) ~= 0
        ...
        %img2proc = [image; zeros(h_padding, width)];
        %h_res = height + h_padding;
        h_res = height- mod(height, h_base) + h_base;
        pad = true;
    else
        h_res = height;
    end
    if mod(width, w_base) ~= 0
        ...
        %img2proc = [image, zeros(height, w_padding)];
        %w_res = width + w_padding;
        w_res = width- mod(width, w_base) + w_base;
        pad = true;
    else
        w_res = width;
    end
