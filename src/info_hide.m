function [image_res1,image_res2,image_res3,decoding_res1,decoding_res2,decoding_res3,info_matrix_height,info_matrix_width] = info_find(image_src,info)
    image = imread(image_src);
    image = rgb2gray(image);%将图像灰度化
    [a,b] = size(image);
    a = a - mod(a,8);
    b = b - mod(b,8);
    image = imresize(image,[a b]);
    load('../resource/JpegCoeff.mat');% DCTAB, ACTAB, QTAB
    % DCT域信息隐藏技术
    [image_res1,image_res2,image_res3,decoding_res1,decoding_res2,decoding_res3] = my_dct_domain_information_hiding_technology(image,DCTAB, ACTAB, QTAB,info);
    % 处理的信息
    info_matrix = de2bi(unicode2native(info),'left-msb')';
    info_matrix_height = size(info_matrix,1);
    info_matrix_width = size(info_matrix,2);
end
% DCT域信息隐藏技术
function [image_res1,image_res2,image_res3,decoding_res1,decoding_res2,decoding_res3] = my_dct_domain_information_hiding_technology(image,DCTAB, ACTAB, QTAB,info)
    [C_tilde,image_height, image_width] = my_jpeg_process(image,QTAB);
    info1 = randi([0, 1], size(C_tilde, 1), size(C_tilde, 2));% 64*6000
    info23 = randi([0, 1], 1, size(C_tilde, 2)); % 6000
    
    info_matrix = de2bi(unicode2native(info),'left-msb')';
    info_matrix = [info_matrix,zeros(size(info_matrix,1),size(C_tilde, 2)-size(info_matrix,2))];
    info_matrix = [info_matrix;zeros(size(C_tilde, 1)-size(info_matrix,1),size(C_tilde, 2))];
    info1 = info_matrix;
    
    info_matrix = de2bi(unicode2native(info),'left-msb')';
    info_matrix = reshape(info_matrix,[1,size(info_matrix,1)*size(info_matrix,2)]);
    info_matrix = [info_matrix,zeros(1,size(C_tilde, 2)-size(info_matrix,2))];
    info23 = info_matrix;
    ...
    disp('width:');
    disp(size(C_tilde, 2));
    [image_res1,dc_stream1,ac_stream1,find_info1,decoding_res1] = my_dct_domain_information_hiding_technology_01(info1,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB);
    [image_res2,dc_stream2,ac_stream2,find_info2,decoding_res2] = my_dct_domain_information_hiding_technology_02(info23,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB);
    [image_res3,dc_stream3,ac_stream3,find_info3,decoding_res3] = my_dct_domain_information_hiding_technology_03(info23,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB);
...
function [image_res1,dc_stream1,ac_stream1,find_info1,decoding_res1] = my_dct_domain_information_hiding_technology_01(info1,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB)
    h = image_height;
    w = image_width;
    hp = h / 8;
    wp = w / 8;
...
function [image_res2,dc_stream2,ac_stream2,find_info2,decoding_res2] = my_dct_domain_information_hiding_technology_02(info23,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB)
    h = image_height;
    w = image_width;
    hp = h / 8;
    wp = w / 8;
...
function [image_res3,dc_stream3,ac_stream3,find_info3,decoding_res3] = my_dct_domain_information_hiding_technology_03(info23,C_tilde,image_height,image_width,QTAB,DCTAB,ACTAB)
    h = image_height;
    w = image_width;
    hp = h / 8;
    wp = w / 8;
